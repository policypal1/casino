/* Lucky Lemons — 3×3 Cylinder Edition (v10)
   - SVG symbols (crisp)
   - Longer spins: ~5s, ~7s, ~10s
   - Diamonds base chance = 5% (shown in Admin)
   - Friendlier payouts and upgrade bias
   - Win modal tiers; Jackpot blast
   - Start with 0 spins
*/
(() => {
  // ---------- Config ----------
  const USERS = ["Will", "Isaac"];
  const DEFAULT_SPINS = 0; // start at zero per request

  // Payouts (25¢). 10¢ = ×0.4. Made friendlier.
  const BASE_PAY_3 = {
    diamond: 4.50,
    seven:   2.80,
    star:    1.70,
  ...
  const BASE_PAY_2_CHERRY = 0.15;
  const BASE_PAY_2_OTHER  = 0.08;
  ...
  // SVG icons (crisp, subtle gloss)
  const ICON_SVGS = {
    diamond: `
      <svg viewBox="0 0 100 100" class="ico-base ico-diamond">
        <defs><linearGradient id="gd" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="var(--a)"/><stop offset="1" stop-color="var(--b)"/></linearGradient></defs>
        <polygon points="20,15 80,15 95,40 50,90 5,40" fill="url(#gd)" stroke="#aee3ff" stroke-width="4" stroke-linejoin="round"/>
        <polyline points="20,15 50,50 80,15" fill="none" stroke="#dff4ff" stroke-width="2" opacity=".7"/>
      </svg>`,
    seven: `
      <svg viewBox="0 0 100 100" class="ico-base ico-seven">
        <defs><linearGradient id="g7" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="var(--a)"/><stop offset="1" stop-color="var(--b)"/></linearGradient></defs>
        <path d="M20 20 H82 L44 86 H22 L58 32 H20 Z" fill="url(#g7)" stroke="#0c3b35" stroke-width="6" stroke-linejoin="round"/>
      </svg>`,
    star: `
      <svg viewBox="0 0 100 100" class="ico-base ico-star">
        <defs><linearGradient id="gs" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="var(--a)"/><stop offset="1" stop-color="var(--b)"/></linearGradient></defs>
        <path d="M50 8 L61 36 L91 36 L66 54 L75 84 L50 66 L25 84 L34 54 L9 36 L39 36 Z" fill="url(#gs)" stroke="#3a2a00" stroke-width="5" stroke-linejoin="round"/>
      </svg>`,
    bell: `
      <svg viewBox="0 0 100 100" class="ico-base ico-bell">
        <defs><linearGradient id="gb" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="var(--a)"/><stop offset="1" stop-color="var(--b)"/></linearGradient></defs>
        <path d="M50 16c-12 0-22 9-22 20v14c0 8-5 13-10 18h64c-5-5-10-10-10-18V36c0-11-10-20-22-20z" fill="url(#gb)" stroke="#6d4c00" stroke-width="5" />
        <circle cx="50" cy="78" r="6" fill="#ffecb3" stroke="#6d4c00" stroke-width="4"/>
      </svg>`,
    grape: `
      <svg viewBox="0 0 100 100" class="ico-base ico-grape">
        <defs><linearGradient id="gg" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="var(--a)"/><stop offset="1" stop-color="var(--b)"/></linearGradient></defs>
        <circle cx="50" cy="18" r="6" fill="#6fbf73"/><rect x="48" y="18" width="4" height="10" fill="#4e8b56"/>
        <g fill="url(#gg)" stroke="#3b2666" stroke-width="4">
          <circle cx="35" cy="50" r="11"/><circle cx="50" cy="50" r="11"/><circle cx="65" cy="50" r="11"/>
          <circle cx="42" cy="64" r="11"/><circle cx="58" cy="64" r="11"/>
        </g>
      </svg>`,
    orange: `
      <svg viewBox="0 0 100 100" class="ico-base ico-orange">
        <defs><linearGradient id="go" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="var(--a)"/><stop offset="1" stop-color="var(--b)"/></linearGradient></defs>
        <circle cx="50" cy="55" r="26" fill="url(#go)" stroke="#6e2a00" stroke-width="5"/>
        <path d="M52 28c10-6 16-6 22-2-6 2-12 5-22 2z" fill="#7ac96d" stroke="#2b5e2c" stroke-width="3"/>
      </svg>`,
    lemon: `
      <svg viewBox="0 0 100 100" class="ico-base ico-lemon">
        <defs><linearGradient id="gl" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="var(--a)"/><stop offset="1" stop-color="var(--b)"/></linearGradient></defs>
        <ellipse cx="52" cy="55" rx="30" ry="22" fill="url(#gl)" stroke="#7a6200" stroke-width="5"/>
        <circle cx="33" cy="55" r="3" fill="#fff" opacity=".5"/>
        <path d="M48 28c-9-5-15-5-21-2 6 2 11 5 21 2z" fill="#7ac96d" stroke="#2b5e2c" stroke-width="3"/>
      </svg>`,
    cherry: `
      <svg viewBox="0 0 100 100" class="ico-base ico-cherry">
        <defs><linearGradient id="gc" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="var(--a)"/><stop offset="1" stop-color="var(--b)"/></linearGradient></defs>
        <path d="M32 34c10 10 22 8 32 0" fill="none" stroke="#2b5e2c" stroke-width="5"/>
        <circle cx="36" cy="62" r="12" fill="url(#gc)" stroke="#5c0b0b" stroke-width="5"/>
        <circle cx="58" cy="62" r="12" fill="url(#gc)" stroke="#5c0b0b" stroke-width="5"/>
        <path d="M50 30 c8-8 16-10 24-10" fill="none" stroke="#2b5e2c" stroke-width="5"/>
      </svg>`
  };

  // ---------- State ----------
  const storeKey = "lucky-lemons-v10-stats";
  let stats = loadStats();
  let currentUser = USERS.includes(localStorage.getItem("ll-v10-user")) ? localStorage.getItem("ll-v10-user") : USERS[0];
  let betCents = Number(localStorage.getItem("ll-v10-bet")) || 25;
  ...
  // More generous upgrade pass for demo feel
  function upgradeRow(row, luck){
    const L = Math.max(0, Math.min(100, luck));
    const u  = Math.min(0.52, L/120); // create a pair
    const u2 = Math.min(0.26, L/280); // upgrade to triple
  ...
  const CELL_H = 150 + 6; // CSS var(--cell-h) + gap
  ...
  function jackpotBlast(amount){
    // Screen flash + shake + extra confetti
  ...
  userSelect.addEventListener("change", ()=>{ currentUser=userSelect.value; localStorage.setItem("ll-v10-user", currentUser); renderStats(); refreshOdds(); hideAdmin(); });
  bet10.addEventListener("click", ()=>{ betCents=10; localStorage.setItem("ll-v10-bet","10"); markBet(); renderPaytable(); refreshOdds(); });
  bet25.addEventListener("click", ()=>{ betCents=25; localStorage.setItem("ll-v10-bet","25"); markBet(); renderPaytable(); refreshOdds(); });
  ...
  function refreshOdds(){
    ...
    // RTP estimate for current bet mode
    const bet = betCents/100;
    const rtp = 100 * (totalPaid / trials) / bet;
    const hitRate = 100 * (trials - none) / trials;
    rtpStats.innerHTML = `Estimated Hit Rate: <b>${hitRate.toFixed(1)}%</b> · Estimated RTP (demo tuning): <b>${rtp.toFixed(1)}%</b>`;
  }
